...
{
  header('location:trafficindex.php');
}
...
if (isset($_POST['btnsubmit'])) {
	...
	$stmt=$conn->prepare("SELECT tra_id,tra_username,tra_password from tbl_traffic where tra_username=:tra_username And tra_password=:tra_password");
	...
	$result = $stmt->setFetchMode(PDO::FETCH_ASSOC); 
	 if($stmt->rowCount()>0)
        {
          $user = $stmt->fetch();
          $_SESSION['id'] = $user['tra_id'];
          $_SESSION['username']= $user['tra_username'];
            $_SESSION['role']='user';
